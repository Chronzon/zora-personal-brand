...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (success && mounted) {
        // --- LOGIKA NAVIGASI ---
        bool hasBrandingData = await onboardingProvider.loadUserData();
        ...
        if (mounted) {
          if (hasBrandingData) {
            Navigator.of(context).pushAndRemoveUntil(
              ...
              (route) => false, // Ini akan menghapus WelcomeScreen dari stack
            );
          } else {
            // User Baru -> Ke NameScreen TANPA tombol back
            Navigator.of(context).pushAndRemoveUntil(
              MaterialPageRoute(
                builder: (_) => const NameScreen(showBackButton: false),
              ),
              (route) => false,
            );
          }
