...
1. $fullName | [Nama Niche 1]
2. $fullName | [Nama Niche 2]
3. $fullName | [Nama Niche 3]
4. $fullName | [Nama Niche 4]
5. $fullName | [Nama Niche 5]
...
""";
  ...
  void _parseAiResponse(String text) {
    ...
    final nameBlockRegex = RegExp(r"Rekomendasi Nama Profil Sosial Media\s*([\s\S]*)", caseSensitive: false);
    ...
    final categoryRegex = RegExp(r"^\s*(?:\d+\.\s*)?Kategori:\s*(.*)", multiLine: true, caseSensitive: false);
    final nicheRegex = RegExp(r"^\s*Micro-niche:\s*(.*)", multiLine: true, caseSensitive: false);
    ...
    final monetizationBlockRegex = RegExp(r"Peluang Monetisasi\s*([\s\S]*?)(?=\n\n|Rekomendasi Nama Profil)", caseSensitive: false);
    ...
    if (monetizationBlockMatch != null) {
      ...
      monetizationText = monetizationBlockMatch.group(1)!
          .split('\n')
          .where((line) => line.trim().isNotEmpty && !line.toLowerCase().contains("dari niche di atas"))
          .map((line) => line.trim())
          .join(', ');
    }
  ...
  void _parsePremiseResponse(String text) {
    ...
    if (listMatch != null) {
      ...
      premiseOptions = premiseBlocks
          ...
          .where((premise) => premise != null && premise.isNotEmpty) // Filter hasil yang null atau kosong
